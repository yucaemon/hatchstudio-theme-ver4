...
if ( ! function_exists( 'amp_get_allowed_shortcode_tags' ) ) {
	function amp_get_allowed_shortcode_tags() {
		$allowed_shortcode_tags = array(
			...
			'st-catgroup',			'st-taggroup',			'st-postgroup',			'st-osusume',		);
...
if ( ! function_exists( 'amp_shortcode_st_card' ) ) {
	function amp_shortcode_st_card( $arg, $content = '' ) {
		...
		if ( st_is_ver_ex() ) {
			...
			$myclass_class    = ( $atts['myclass'] !== '' ) ? $atts['myclass'] : '';
			$type_value       = ( $atts['type'] !== '' ) ? $atts['type'] : '';
			$thumbnail_value  = ( $atts['thumbnail'] !== '' ) ? $atts['thumbnail'] : '';
			...
			if ( ( trim ( $thumbnail_class ) === '' ) && ( isset ( $type_value ) && $type_value === 'wide' ) ) {
			...
			while ( $post_query->have_posts() ) {
				...
				?>
					...
					<div class="st-cardbox-content <?php echo esc_attr(  $myclass_class ); ?>">
						...
						<?php if ( ! $thumbnail_class && has_post_thumbnail() ):
						?>
				...
				<?php } else { ?>
					...
					<div class="kanren st-cardbox <?php echo esc_attr(  $thumbnail_size_class . $thumbnail_class . $myclass_class ); ?>">
						<?php if ( $card_label !== '' || $fontawesome_html !== '' ):
						?>
...
if ( !function_exists( 'amp_shortcode_st_mybutton' ) ) {
	function amp_shortcode_st_mybutton( $atts, $content = null ) {
		...
		$target_set   = ( $target !== '' ) ? ' target="_blank"' : '';
		$fontawesome_html = ( $fontawesome !== '' ) ? '<i class="st-fa ' . esc_attr( $fontawesome ) . ' st-css-no" aria-hidden="true"></i>' : '';
		$fontawesome_after_html = ( $fontawesome_after !== '' ) ? '<i class="st-fa st-svg-after ' . esc_attr( $fontawesome_after ) . ' st-css-no" aria-hidden="true"></i>' : '';
...
if ( !function_exists( 'amp_shortcode_st_mybox' ) ) {
	function amp_shortcode_st_mybox( $atts, $content = null ){
		...
		$fontawesome_css = $fontawesome !== '' ? '<i class="st-fa  ' . esc_attr($fontawesome) . '" aria-hidden="true"></i>' : '';
...
if ( !function_exists( 'amp_shortcode_st_flexbox' ) ) {
	function amp_shortcode_st_flexbox( $atts, $content = null ) {
        ...
        $title_html        = ( $title !== '' ) ? '<p class="st-header-flextitle">' . $fontawesome_html . $title . '</p>' : '';
...
if ( !function_exists( 'amp_shortcode_st_google_icon' ) ) {
	function amp_shortcode_st_google_icon( $atts, $content = null ) {
		...
		$googleicon_html = ( $googleicon !== '' ) ? '<i class="material-icons">'. esc_attr( $googleicon ) .'</i>' : '';
